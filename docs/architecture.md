...
| Step | Rule | Owner |
|------|------|--------|
| Emit mutations | Every write path offers `BrowseMutation` (`UpsertNote`, `UpsertFolder`, `DeleteNote`, `DeleteFolder`) via `PendingBrowseSync`. | `CreateNoteViewModel`, `NoteViewModel`, `BrowserViewModel`, callers |
...
Theme preferences in DataStore include color theme, appearance, `colorizedHeadings` / `headingColorPalette` (read-mode heading colors), and `colorizeCheckboxStatus` (optional red/green tint for GFM task rows in read view and on widgets). Heading and task color toggles trigger `NoteWidgetRefresh.refreshAll()` when changed in Settings.
...
Empty folders can be deleted with `DELETE /api/v1/folders/{path}` (no `If-Match`). The server removes the on-disk directory only when it has no entries (`409` / `folder_not_empty` when non-empty), then starts a full index refresh. Android deletes through `VaultRepository.deleteFolder` and a trash icon on `BrowserScreen` when browsing inside a writable folder, with confirmation. On success, `PendingBrowseSync` offers a folder delete mutation and the client navigates up one level (see [Stale index, writes, and client sync](#stale-index-writes-and-client-sync)).
